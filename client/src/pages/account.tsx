...
const AccountPage = () => {
  ...
  const [SAPIPASSWORD, setSAPIPASSWORD] = React.useState<string>("");
  ...
  const getAccountInformation = () => {
    const asyncFun = async () => {
      ...
      const { data } = await axios.post<IAPIResponse>(
        ...
        { credential: SAPIKEY, password: SAPIPASSWORD }
      );
  ...
  const performTransaction = (amount: number | "") => {
    const asyncFun = async () => {
      ...
      const { data } = await axios.post<IAPIResponse>(
        ...
        { credential: SAPIKEY, password: SAPIPASSWORD, amount: amount }
      );
      setNTransaction(0);
      if (!data.success) {
    ...
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        ...
        <input
          ...
          type={"text"}
          value={SAPIPASSWORD}
          onChange={(e) => setSAPIPASSWORD(e.target.value)}
        />
